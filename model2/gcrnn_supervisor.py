...
from model2.gcrnn_model import GCRNNModel
from model2.loss import masked_mae_loss
from model2.CBAM import SpatialTemporalAttention  # 假设您的时空注意力模块文件名为 spatial_temporal_attention.py
...
class GCRNNSupervisor:
    def __init__(self, adj_mx, **kwargs):
        ...
        # 初始化时空注意力模块
        gate_channels = self.input_dim  # 假设 input_dim 是数据的通道数
        reduction_ratio = 1  # 可根据需要调整
        self.spatial_temporal_attention = SpatialTemporalAttention(gate_channels=gate_channels, reduction_ratio=reduction_ratio)
    ...
    def save_model(self, epoch):
        ...
        作用：保存模型
        """
        # 修改保存路径为 models/gcrnn/beijing
        save_dir = 'models/gcrnn/beijing/'
        ...
        # 保存模型，路径基于 save_dir
        torch.save(config, os.path.join(save_dir, 'epo%d.tar' % epoch))# 保存模型，第一个参数为 要保存的对象；第二个参数为 保存文件名
        self._logger.info("Saved model at {}".format(epoch)) # 保存日志
        return os.path.join(save_dir, 'epo%d.tar' % epoch)# 将保存的模型的文件路径返回
    ...
    def load_model(self):
        ...
        # 修改加载路径为 models/gcrnn/beijing
        load_dir = 'models/gcrnn/beijing/'

        # 确认要加载的模型文件是否存在
        model_path = os.path.join(load_dir, 'epo%d.tar' % self._epoch_num)

        # 断言：要是路径存在 则继续，要是找不到 则抛出异常，显示后面的错误提示
        assert os.path.exists(model_path), 'Weights at epoch %d not found in %s' % (self._epoch_num, model_path)
        checkpoint = torch.load(model_path, map_location='cpu')
        self.gcrnn_model.load_state_dict(checkpoint['model_state_dict'])
    ...
    def _setup_graph(self):
        ...
        """
        with torch.no_grad():
            self.gcrnn_model = self.gcrnn_model.eval()
            ...
            for _, (x, y) in enumerate(val_iterator):
                ...
                # 调用模型时返回隐藏状态和记忆单元
                output, attention_weights = self.gcrnn_model(x)
    ...
    def evaluate(self, dataset='val', batches_seen=0):
        ...
        with torch.no_grad():
            ...
            for _, (x, y) in enumerate(val_iterator):
                ...
                # 修改处: 调用模型时解包 output 和 attention_weights
                output, _ = self.gcrnn_model(x)
    ...
    def _train(self, base_lr,
        ...
        # 参数：优化器、调整学习率的间隔（list，递增）、学z习率调整倍数（这里默认了0.1,也就是10倍）
        ...
        for epoch_num in range(self._epoch_num, epochs):
            ...
            for _, (x, y) in enumerate(train_iterator): # train_iterator是一个生成器，可以迭代，每次迭代则切片 取一次数据，好处是节约内存啊
                ...
                # 调用模型并返回 hx 和 cx
                # 修改处: 调用模型时解包 output 和 attention_weights
                output, _ = self.gcrnn_model(x, y, batches_seen)
    ...
    def _prepare_data(self, x, y):
        ...
        # Step 2: 通过时空注意力模块
        x = self.spatial_temporal_attention(x)  # 在内部会自动调整到 (B, F, T, N) 并返回 (T, B, N, F)
